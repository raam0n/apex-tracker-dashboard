...
const processSnap = async (imagePath) => {
    console.log('👁️ Vision Engine: Escaneando pantalla completa...');
    const worker = await createWorker('eng');

    try {
        let image = await Jimp.read(imagePath);

        // 1. PRE-PROCESAMIENTO GLOBAL
        image.greyscale().contrast(0.6);
        const buffer = await image.getBuffer('image/png');

        // 2. EXTRACCIÓN FULL TEXT
        ...
        console.log(`📄 Texto crudo guardado en: ${rawTextPath}`);

        // --- EXTRACCIÓN DE PLACEMENT ---
        const placementPattern = /PLACED\s*[\r\n]+\s*#\s*(\d{1,2})\s*[\r\n]+\s*WITH SQUAD/i;
        let placementMatch = text.match(placementPattern);

        if (!placementMatch) {
            placementMatch = text.match(/PLACED\s*[\r\n]*\s*#\s*(\d{1,2})/i);
        }
        ...
        // --- EXTRACCIÓN DE K/A/K (Lógica Estructural Avanzada) ---
        let kakLines = [];
        ...
        const snapshot = {
            ...
            rawText: text, // Necesario para el frontend
            results: {
                placement: placement,
